...
class TestBookRepository:
    ...
    def test_delete_book(self, test_db):
        ...
        book = create_book(test_db, BookIn(title="To Delete", author="Author", isbn="9780385533225"))
...
class TestBookAPI:
    ...
    def test_create_book_missing_isbn(self, client):
        """Test POST /api/books/ without isbn returns 422 (validation error)"""
        response = client.post("/api/books/", json={"title": "No ISBN", "author": "Author"})
        assert response.status_code == 422

    def test_create_book_invalid_isbn(self, client):
        """Test POST /api/books/ with an invalid isbn returns 422"""
        response = client.post("/api/books/", json={"title": "Bad ISBN", "author": "Author", "isbn": "notanisbn"})
        assert response.status_code == 422
    ...
    def test_get_books_pagination(self, client):
        """Test GET /api/books/ pagination with skip and limit"""
    ...
    def test_get_book(self, client):
        ...
        assert response.json()["isbn"] == TEST_BOOKS[0]["isbn"]
